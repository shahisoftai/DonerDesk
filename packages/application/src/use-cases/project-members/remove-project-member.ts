...
export class RemoveProjectMemberHandler {
  ...
  async handle(ctx: AuthenticatedContext, memberId: string): Promise<Result<void, DomainError>> {
    ...
    if (member.userId === ctx.tenant.userId) {
      return { ok: false, error: DomainError.validation("You cannot remove yourself from a project") };
    }
