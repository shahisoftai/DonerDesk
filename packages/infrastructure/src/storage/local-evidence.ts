 ...
 * Delegates the actual blob writes to the existing LocalStorage adapter. Files
 * are stored under a tenant-scoped `tenantId/evidence` key so the `/v1/files/:key`
 * route can enforce tenant isolation by prefix.
 */
