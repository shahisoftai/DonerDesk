...
| `PUT` | `/superadmin/tiers/:planCode` | Update a tier's global feature allocation (`name`, prices, `trialDays`, `enabled`, partial `limits`) — writes a `PlanCatalogOverride`; `null` limit buckets mean unlimited for that tier (2026-08-17; `trialDays` is dormant catalog data since the 14-day trial offer was removed 2026-08-18) |
...
| `POST` | `/superadmin/tenants/:id/tier/reset` | End the tenant's MANUAL overrides so it falls back to subscription / Starter entitlement (grants are closed, never deleted) (2026-08-17) |
