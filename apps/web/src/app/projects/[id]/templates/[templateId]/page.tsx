...
export default async function TemplateEditor({ params }: { params: Promise<{ id: string; templateId: string }> }) {
  ...
  if (!tpl) return <div className="p-8 text-sm text-slate-500">Template not found.</div>;
  return (
    <main className="mx-auto max-w-4xl px-6 py-8">
      <h1 className="text-2xl font-bold">{tpl.templateName}</h1>
      <p className="text-sm text-slate-500">Review and edit AI-extracted sections before saving.</p>
