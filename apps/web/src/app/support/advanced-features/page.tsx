    ...
    <div className="landing-tech min-h-screen overflow-x-hidden bg-slate-950 text-slate-100">
      ...
      <div className="border-b border-white/5 bg-slate-950/50 px-6 py-3">
        <div className="mx-auto flex max-w-7xl items-center gap-1.5 text-xs text-slate-400">
          ...
          <span>›</span>
          <span className="text-slate-200">Advanced Features</span>
        </div>
      </div>
      <div className="mx-auto flex max-w-7xl gap-0 px-6 py-8">
        ...
        <main className="min-w-0 flex-1 lg:px-8">
          <div className="mb-8">
            <div className="inline-flex rounded-xl border bg-gradient-to-br from-violet-500/20 to-purple-500/10 px-4 py-2 border-violet-400/30">
              <span className="text-violet-300">{cat.icon}</span>
              <span className="ml-2 text-sm font-semibold text-white">{cat.name}</span>
            </div>
            <h1 className="mt-4 text-3xl font-extrabold text-white">Advanced Features</h1>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {cat.articles.map((art) => (
              <Link
                key={art.href}
                href={art.href}
                className="group rounded-xl border border-white/10 bg-white/[0.03] p-5 transition hover:-translate-y-0.5 hover:border-violet-400/30 hover:bg-white/[0.05]"
              >
                <div className="flex items-start justify-between">
                  <h3 className="font-semibold text-white group-hover:text-violet-300">{art.title}</h3>
                  <svg viewBox="0 0 20 20" fill="currentColor" className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-600 transition group-hover:translate-x-0.5 group-hover:text-violet-400">
