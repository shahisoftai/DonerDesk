...
const FILE_MAP: Record<string, string> = {
  ...
  "templates": "advanced-features-templates.md",
  "advanced-features": "advanced-features-ai-settings.md",
};
...
export default async function AdvancedFeaturesArticlePage({
  ...
  const cat = WIKI_CATEGORIES[3]!;
  const artEntry = cat.articles.find((a) => a.href === `/support/advanced-features/${article}`);
    ...
    <div className="landing-tech min-h-screen overflow-x-hidden bg-slate-950 text-slate-100">
      ...
      <div className="border-b border-white/5 bg-slate-950/50 px-6 py-3">
        <div className="mx-auto flex max-w-7xl items-center gap-1.5 text-xs text-slate-400">
          ...
          <span>›</span>
          <Link href="/support/advanced-features">Advanced Features</Link>
          <span>›</span>
        ...
        <main className="min-w-0 flex-1 lg:px-8">
          <article className="prose prose-invert prose-slate max-w-none">
            ...
            {artEntry?.description && (
              <p className="text-slate-400 text-sm mb-6 border-l-2 border-violet-500/40 pl-3">
                {artEntry.description}
              </p>
            )}
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-6">
              <ReactMarkdown
                ...
                components={{
                  ...
                  a: ({ href, children }) => <a href={href} className="text-violet-400 hover:text-violet-300 underline" target="_blank" rel="noopener">{children}</a>,
                  strong: ({ children }) => <strong className="text-white font-semibold">{children}</strong>,
                  code: ({ children }) => <code className="bg-white/5 text-violet-300 px-1.5 py-0.5 rounded text-sm">{children}</code>,
                  blockquote: ({ children }) => <blockquote className="border-l-4 border-violet-500/40 pl-4 italic text-slate-400 my-4">{children}</blockquote>,
            ...
            <div className="mt-8 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.02] p-4">
              <div />
              <Link href="/support/advanced-features" className="text-sm text-violet-400 hover:text-violet-300 transition">
