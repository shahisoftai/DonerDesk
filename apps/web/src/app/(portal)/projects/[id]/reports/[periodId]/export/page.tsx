...
export default async function ExportPage({ params }: { params: Promise<{ id: string; periodId: string }> }) {
  ...
  return <div className="animate-fade-in"><header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-brand-600 dark:text-brand-400">Controlled snapshot</p><h2 className="mt-1 text-2xl font-bold">Export center</h2><p className="mt-1 text-sm text-slate-500">Review warnings, select files, and preserve immutable export history.</p></div><Link className="btn-secondary" href={`/projects/${id}/reports/${periodId}`}>Back to report</Link></header>
