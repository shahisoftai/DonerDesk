  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        className="relative grid h-11 w-11 place-items-center rounded-xl border border-slate-300 bg-white/70 text-slate-600 shadow-sm transition hover:border-brand-400 hover:text-brand-600 dark:border-white/15 dark:bg-white/5 dark:text-slate-300 dark:hover:border-brand-400/60"
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <div role="menu" className="absolute right-0 z-30 mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-white/10 dark:bg-slate-900">
